...
void HuffmanCoding::traverse(Node* r, string str) {
...
int HuffmanCoding::binToDec(string inStr) {
    int res = 0;
    for (auto c : inStr) {
        res = res * 2 + c - '0';
    }
...
string HuffmanCoding::decToBin(int inNum) {
    string temp = "", res = "";
    while (inNum > 0) {
        temp += (inNum % 2 + '0');
        inNum /= 2;
...
void HuffmanCoding::buildTree(char a_code, string& path) {
    Node* curr = root;
...
void HuffmanCoding::createCodes() {
    traverse(root, "");
}

void HuffmanCoding::saveEncodedFile() {
    inFile.open(inFileName, ios::in);
...
void HuffmanCoding::saveDecodedFile() {
